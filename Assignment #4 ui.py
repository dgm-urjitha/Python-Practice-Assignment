#!/usr/bin/env/python3
# LAKSHMI URJITHA DHADIGAM - ASSIGNMENT 4 - 1223270
...
def update_movie():
    ...
    if movie == None:
        print("There is no movie with that ID. Movie NOT Updated.\n")
    else:
        name        = input("Name: ")
        year        = get_int("Year: ")
        minutes     = get_int("Minutes: ")
        ...
        category = db.get_category(category_id)
        if category == None:
            print("There is no category with that ID. Movie NOT updated.\n")
        else:
            movie = Movie(name=name, year=year, minutes=minutes, category=category, id=movie_id)
            db.update_movie(movie)
            print(f"{name} was updated to database.\n")
